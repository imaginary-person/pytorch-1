...
// Floating and complex types (minus Half and BFloat16) allow for optimized kernels
template <typename scalar_t, typename exp_scalar_t>
void pow_tensor_scalar_optimized_kernel(TensorIteratorBase& iter, const exp_scalar_t exp) {
  ...
  if (exp == 0.5) {
    ...
    cpu_kernel_vec(
        iter,
        [](scalar_t base) -> scalar_t { return scalar_t(1.0) / base; },
        [](Vec base) -> Vec { return base.reciprocal(); });
  } else if (exp == -2.0) {
    cpu_kernel_vec(
        iter,
        [](scalar_t base) -> scalar_t { return scalar_t(1.0) / (base * base); },
...
void pow_tensor_scalar_kernel(
  ...
  if (dtype == ScalarType::Float || dtype == ScalarType::Double) {
    AT_DISPATCH_FLOATING_TYPES(dtype, "pow", [&]() {
      pow_tensor_scalar_optimized_kernel<scalar_t>(
